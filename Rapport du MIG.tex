...
En effet, ces centrales hydrauliques reposent sur une succesion de conversion énergétique. L'énergie potentielle de l'eau est stockée en hauteur notamment à travers des barrages retenant l'eau. Cette eau rejoint, dans un second temps, à travers des conduites forcées, la centrale hydraulique en contrebas, l'énergie potentielle se convertissant en énergie cinétique. Cette dernière permet de mettre en mouvement les turbines afin de produire l'énergie mécanique, nécessaire à un alternateur pour produire de l'électricité. La puissance de la centrale dépend de la hauteur de chute et du débit de l'eau. L'eau turbinée, ayant perdue sa puissance, rejoint finalement la rivière par un canal de fuite. \\

Ainsi, il existe trois principaux types de centrales hydrauliques : 
\begin{itemize}
...
\begin{itemize}
    ...
    \item Modélisation data-driven de la production hydroélectrique et de la demande en electricité à partir de données climatiques.
    \item Étude prospective du système électrique national selon plusieurs scénarios.
\end{itemize}
...
\subsubsection{Modélisation physique des variables}
...
\subsubsection{Le fléau de la dimension}
...
\subsubsection{Stratégies de réduction de dimension}
...
Concernant les résultats du modèle, on peut toujours tracer la prédiction réalisée pour l'année 2023. 

\begin{figure} [H]

\centrage

\includegraphics[width=0.6\textwidth]{xgboost.png}

\caption{Comparaison prédiction et réalité}

\étiquette{fig:resultats xgboost}

\end{figure} 


Premièrement, on peut se réjouir de la bonne allure globale de la courbe prédite qui suit les tendances globales et quelques variations à l'échelle journalière de la courbe réelle. Le modèle parvient à interpréter les variations de température et précipitations à plusieurs échelles pour en prédire un facteur de charge journalier qui reste cohérent. La sécheresse de février 2023, une anomalie jamais rencontrée dans le jeu d'entraînement parvient a être un peu perçue par le modèle. 

Cependant, le modèle tendant à surrestimer les valeurs prédites, on a ajusté verticalement la prédiction en calculant un biais comme précédemment. 
Cette méthode, si elle fournit de bons résultats à court terme peut présenter des limites pour des projections à plus long terme : puisque la méthode de calcul dépend des prédictions passées du modèle.
Pour conclure sur cette partie, ce modèle XGBoost est le meilleur modèle de machine learning que l’on a pu construire à partir de nos données d’entrées. Cependant, les erreurs de prédiction peuvent sembler toujours assez élevées. Pour expliquer cela, il faut comprendre qu’un algorithme de Machine Learning perçoit le jeu de données comme une nuage de points dans un espace de dimension très élevée (ce qui s’affranchit de la dépendance temporelle entre les données, ce qui est dommage). C’est pour cette raison que l’on a cherché à développer d’autres modèles d’apprentissage prenant en compte cette dépendance temporelle, à travers le Deep Learning.
...
\paragraph{La théorie mathématique}
Les réseaux de neurones artificiels «~classiques~», aussi appelés réseaux \emph{feedforward} (ou perceptrons multicouches, MLP), sont des modèles paramétriques qui approximent une fonction $f_\theta : \mathbb{R}^d \rightarrow \mathbb{R}^k$ en composant plusieurs transformations linéaires et non linéaires. Un réseau à $L$ couches calcule successivement, à partir d’une entrée $x$, des activations $h^{(0)} = x$ puis, pour $l=1,\dots,L$, 
\[
z^{(l)} = W^{(l)} h^{(l-1)} + b^{(l)}, 
\qquad 
h^{(l)} = \phi^{(l)}\!\big(z^{(l)}\big),
\]
où $W^{(l)}$ et $b^{(l)}$ sont les paramètres appris, et $\phi^{(l)}$ une fonction d’activation (ReLU, $\tanh$, etc.) introduisant la non-linéarité. La dernière couche produit une sortie $\hat{y}=h^{(L)}$ (par exemple, une \emph{softmax} en classification) qui est comparée à la cible $y$ via une fonction de perte $\mathcal{L}(\hat{y},y)$. L’apprentissage consiste à minimiser l’erreur moyenne sur un jeu de données $\{(x_i,y_i)\}_{i=1}^n$ en ajustant $\theta=\{W^{(l)},b^{(l)}\}$ par descente de gradient (souvent stochastique)~: les gradients $\nabla_\theta \mathcal{L}$ sont calculés efficacement grâce à la rétropropagation (\emph{backpropagation}), puis les paramètres sont mis à jour selon $\theta \leftarrow \theta - \eta \nabla_\theta \mathcal{L}$, où $\eta$ est le taux d’apprentissage.
...
Les réseaux de neurones \emph{feedforward} (tels que les MLP) constituent une base solide en apprentissage supervisé, mais montrent une limite structurelle dès lors qu’ils sont appliqués à des \textbf{séries temporelles} : ils ne représentent pas explicitement la \textbf{dépendance séquentielle}. Un MLP traite chaque observation comme un vecteur indépendant et, sauf recours à une ingénierie de variables (retards, fenêtres glissantes, agrégations, encodage temporel), il n’intègre pas naturellement le fait que \emph{l’ordre} et le \emph{contexte} des mesures portent de l’information. Cette absence de mémoire interne complique la capture de dynamiques telles que l’inertie, les effets retardés, les saisonnalités ou certaines interactions temporelles, et rend la performance plus sensible au choix de la fenêtre d’observation et aux \emph{features} construites.

\paragraph{Réseaux récurrents et état caché}
Pour intégrer cette dimension temporelle, nous retenons une architecture \textbf{récurrente} (\emph{Recurrent Neural Network}, RNN), qui introduit une forme de mémoire via un \textbf{état caché} $h_t$ mis à jour à chaque pas de temps. À l’instant $t$, la représentation dépend à la fois de l’entrée courante $x_t$ et du contexte accumulé dans $h_{t-1}$, ce qui permet de modéliser des dépendances de court terme sans spécifier explicitement, en entrée, une liste de retards. Formellement, un schéma récurrent simple s’écrit typiquement
\[
h_t = \phi\!\big(W_x x_t + W_h h_{t-1} + b\big),
\]
où $\phi$ est une non-linéarité et $(W_x,W_h,b)$ des paramètres appris.

\paragraph{LSTM et apprentissage des dépendances longues}
Nous utilisons plus spécifiquement une cellule \textbf{LSTM} (\emph{Long Short-Term Memory}), conçue pour atténuer une difficulté classique des RNN : la \textbf{disparition du gradient} lors de la rétropropagation à travers de longues séquences, qui freine l’apprentissage de dépendances à long terme. Le LSTM introduit un état de mémoire $c_t$ et un mécanisme de \textbf{portes} (\emph{gates}) qui contrôlent le flux d’information : une \emph{porte d’oubli} sélectionne ce qui est conservé du passé, une \emph{porte d’entrée} régule l’intégration de nouvelles informations, et une \emph{porte de sortie} détermine la partie de la mémoire exposée à l’état caché. Cette régulation stabilise l’optimisation et permet de capturer des motifs temporels persistants (tendances, régimes, effets retardés).

\paragraph{Stacked LSTM et représentations hiérarchiques}
Afin d’augmenter la capacité de représentation, nous adoptons un \textbf{Stacked LSTM}, où plusieurs couches LSTM sont empilées : la sortie d’une couche sert d’entrée à la suivante. Cette profondeur favorise l’apprentissage \textbf{hiérarchique} des dynamiques temporelles : les premières couches captent souvent des régularités locales (variations rapides, corrélations de court terme), tandis que les couches supérieures modélisent des structures plus \textbf{abstraites} et \textbf{non linéaires} (tendances globales, interactions complexes entre variables, dépendances plus longues). En pratique, cette architecture améliore la flexibilité du modèle au prix d’un coût de calcul accru et d’un besoin plus marqué en régularisation (par exemple \emph{dropout}, arrêt anticipé) pour limiter le sur-apprentissage.
...
Nous avons voulu utiliser les données obtenues pour 2050 pour faire tourner le modèle Times, et apprendre comment différents paramètres impactent le mix électrique de 2050. Ainsi, dans une démarche de prospective, nous avons mis en place deux scénarios, que nous avons nommés “optimiste” et “pessimiste”. 
Ils correspondent à différents types d'interventions gouvernementales pour l’écologie. \\

Avant de voir plus en détail ces deux scénarios, voici un tableau qui récapitule les paramètres que nous avons fixés dans chacun des deux scénarios : \\
...
Dans le scénario “optimiste” on fixe la demande électrique à la valeur qui a été obtenue sur l'année 2023. Il est difficile d’estimer en pratique quelle sera la variation de la demande électrique dans les prochaines années car elle sera affectée par l’électrification des usages, l’optimisation des procédés actuels, les nouvelles technologies mises en place, et par un potentiel objectif de sobriété. 

On garde de plus la taxe carbone à la valeur actuelle fixée par le gouvernement, et on force ainsi le modèle à respecter l’objectif de neutralité carbone du mix électrique d’ici 2050. Cela correspond simplement à suivre les objectifs théoriques actuels du gouvernement. 
Les derniers paramètres que nous avons modifiés sont les “capacity credit” qu’applique RTE à différentes énergies renouvelables. Ce “credit” correspond au pourcentage de la capacité installée d'une centrale à laquelle RTE fait confiance à tout instant, et notamment aux moments critiques (par exemple lors d’un pic de demande, comme on peut en avoir lors d’un soir d’hiver, dans une période froide). 
Plus il est important pour une source d'énergie, plus TIMES va la considérer comme fiable, et donc comme rentable. Cela nous permet d’obtenir les résultats suivants : \\
[graphe]
C’est un scénario assez similaire à ce que la France met en place actuellement donc on obtient un mix assez similaire à celui actuel, ce qui le rend facile à analyser. 
La partie hydraulique correspond à la capacité maximale installable. La partie nucléaire, rendue compétitive par la taxe carbone, fait le gros de la production électrique. 
La partie gaz, quant à elle, s’occupe de la variabilité du système : elle assure la production pour pallier à l’intermittence des EnR, et aux incertitudes du système énergétique en cas de pic de demande électrique. \\

[retour du tableau]
Dans le scénario pessimiste, l'électrification n’est pas compensée par une sobriété ou par une optimisation, donc la demande d'électricité augmente d’un pour cent par an, ce qui correspond à une augmentation de 30\% de la demande d’ici 2050. 
De plus, le gouvernement de ce scénario supprime la taxe carbone, ce qui change les rapports de prix entre les différents moyens de production d’électricité. 
Nous avons choisi des “capacity credits” qui se rapprochent de ceux fixés par l'Allemagne actuellement. On obtient ainsi ce graphique plus étonnant : \\
[graphe]
Le nucléaire, par exemple, est tombé à 6 pourcent. En effet, il est relativement plus cher que les autres sources d’électricité (120€ le MWh électrique d’après la Cour des Comptes, contre 35 à 45 euros le MWh électrique au gaz, et 20 à 25 euros pour le charbon).
Ainsi, ici, c’est le solaire qui joue le rôle de gros producteur. Cela peut paraître surprenant quand on pense à son “capacity credit” de 0\%, et cela nous permet de mieux comprendre les critères de TIMES. \\
De fait, le logiciel considère qu’il faut une certaine capacité totale, ce qu’on a décrit comme “le gros de la production électrique”. Dans ce cas-ci, le solaire est une capacité pour les moments où le système électrique est en tension, qui elle prend en compte les “capacity credits”, ici le charbon et le gaz notamment. 
On remarque cependant que les centrales hydrauliques sont toujours intéressantes économiquement, même sans taxe carbone. \\
Cependant si ce scénario est intéressant pour en apprendre plus sur TIMES, son fonctionnement, et sur le système électrique, il n’est pas vraiment réaliste. Ce mix ne serait pas possible, notamment politiquement, mais aussi parce que les ressources en charbon ne sont pas infinies. 
On arrive ici aux limites de la linéarité du modèle TIMES; il considère certaines matières premières comme infinies, et ne prend ainsi pas en compte le fait que leur prix augmente en cas de trop forte consommation, notamment s’il faut les importer. \\
