...
\usepackage{mathrsfs}
\usepackage{biblatex}
\usepackage[backend=biber, style=authoryear]{biblatex} % Exemple de style
\addbibresource{Bibliographie.bib} % <--- Assurez-vous que ce nom de fichier est correct
\usepackage{minted}
\usepackage{xcolor}
\usepackage{amsmath}
\usepackage{amssymb}
...
A l’heure où la question du climat et de l’énergie s'impose à nous, le lien entre les deux est de plus en plus étudié et exploré. Nous pouvons en effet désormais trouver des modèles énergétiques qui intègrent la variabilité climatique, ce qui n’était pas le cas il y a quelques années. Le mix énergétique français repose principalement sur l'électricité dont le nucléaire, qui représente $70 \%$ de la production électrique, en est le principal contributeur. Par ailleurs, l'énergie renouvelable prend une place de plus en plus prépondérante avec $24 \%$ de la production énergétique primaire nationale. Mais les variables climatiques ont une grande influence sur leur efficacité productive. En particulier, la variabilité climatique est particulièrement intéressante pour le domaine de l'hydroélectricité, qui représente près de $14 \%$ de la production française et qui dépend directement des précipitations par exemple. Cette dépendance rend la production vulnérable aux aléas météorologiques, comme les sécheresses estivales qui réduisent les débits fluviaux. Dans un contexte de changement climatique, où la fréquence et l'intensité de ces événements extrêmes risquent d'augmenter, anticiper la disponibilité de la ressource hydraulique est crucial pour assurer la sécurité d'approvisionnement et l'équilibre du réseau électrique français.\\
...
De manière plus concrète, nous avons donc modélisé,,en étudiant le cycle de l'eau, la puissance électrique que pourront fournir les barrages en France, en connaissant le volume de précipitations ainsi que la température. On réalise ces prédictions via le facteur de charge (CF), défini ainsi : \\ $$ CF = \frac{P_{produite}}{P_{capacit\acute e \ install\acute ee}}$$

Enfin, pour la partie prospective, nous nous appuyons sur un modèle d’optimisation de type bottom-up issu de la famille TIMES appliqué au système électrique français. Cet outil propose une représentation fine des infrastructures et des dynamiques de fonctionnement à l’échelle infra-journalière, afin d’éclairer les trajectoires d’évolution du système à long terme. Dans ce cadre, nous pouvons quantifier l’influence de la variabilité climatique sur la consommation d’électricité ainsi que sur les potentiels solaire, éolien et hydraulique, au sein de scénarios contrastés du futur mix électrique européen à l’horizon 2050. Ces scénarios sont construits selon différents degrés d’ambition en matière de décarbonation et intègrent des hypothèses différenciées concernant la demande future et la disponibilité des technologies.
...
En ce qui concerne la résolution spatiale, les données climatiques sont agrégées au niveau NUTS2 (régions statistiques utilisées pour l’application des politiques régionales) ou au niveau national.\\
...
\begin{itemize}
    \item la saisonnalité analysée sur les courbes de température et de facts de charge (CF), qui rend compte de données cycliques 
    \item la fonte des neiges au printemps,  source d'eau supplémentaire dans le cycle de l'eau, qui se traduit par l'ajout de variables retardées (lag).
...
Le \textbf{\textit{training set}}, qui représente généralement 60 à 80\% des données, sert à ajuster les paramètres du modèle. Le \textbf{\textit{validation set}} est utilisé pour optimiser les hyperparamètres et déterminer la meilleure configuration avant d’examiner les données de test. Enfin, le \textbf{\textit{test set}}, plus réduit, n’est utilisé qu’une seule fois à la fin du processus afin d’obtenir une évaluation non biaisée des performances du modèle sur des données totalement nouvelles.\\
...
La première, Random Forest, est une méthode d’apprentissage basée sur des arbres de décision. Elle améliore l’approche des arbre de decision classiques en construisant un ensemble d’arbres décorrélés, chacun entraîné sur un échantillon de données et un sous-ensemble aléatoire de variables. Cette technique réduit fortement le risque de surapprentissage et augmente la robustesse du modèle. Elle fournit généralement de bonnes performances même lorsque les relations entre variables sont complexes ou non linéaires.
...
NNous nous sommes ensuite penchés sur une approche différente et plus complexe : le Deep Learning. C'est un autre sous-domaine de l'apprentissage automatique qui utilise des réseaux de neurones artificiels composés de nombreuses couches pour analyser des données et prendre des décisions. 
...
\begin{figure}[!htp]
    ...
    \caption{Facteurs de Charge 2023}
    ...
    \caption{Facteurs de Charge 2022}
...
\begin{figure}[H]
    ...
    \includegraphics[width=0.5\textwidth]{bar_plot_erreur.jpeg} 
    \caption{Comparaison des erreurs pour chaque modèle}
    \label{bar plot errors}
\end{figure}
...
\section{Conclusion}
Dans le cadre de ce projet, nous avons ainsi pu à partir des données de températures et de precipitations en France 
entre 2015 et 2023, créer différents modèles d'apprentissage automatique 
afin de prédire les facteurs de capacités des centrales hydrauliques au fil de l'eau. 
Afin d'améliorer la précision de nos modèles, un premier travail de préparation des données a ainsi été effectué pour 
représenter les dépendances d'une centrale hydraulique aux grandeurs climatiques. Nous avons alors pu entraîner 
nos différents modèles de machine learning et de deep learning sur ces données. 
Après comparaison de ces modèles à partir de différentes erreurs et des courbes confrontant les facteurs de capacité 
réels et prédits, le modèle LSTM a été déterminé comme étant le plus efficace. 
Ce modèle a donc été choisi pour réaliser une étude prospective à horizon 2050. 
Pour celle-ci, les scénarios que nous avons testés correspondent à des cas extrêmes. 
Les issues proposées n'ont pas pour but de prédire exactement le mix énergétique futur, 
et ne correspondront donc probablement pas à notre réalité.
Néanmoins, elles permettent "d'encadrer" ce qui pourrait advenir dans le cadre du scénario tendanciel, et de voir ce à quoi certaines décisions importantes, 
comme la suppression de la taxe carbone dans le cadre du scénario pessimiste, pourraient nous contraindre à mettre en place.
