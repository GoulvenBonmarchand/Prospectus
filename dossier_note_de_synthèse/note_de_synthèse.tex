...
En effet, il existe deux principaux types de centrales hydrauliques: les centrales à barrages (hydroelectric dam) de moyenne ou haute chute, caractérisées par une implantation dans des régions montagneuses, un débit faible à moyen et une hauteur de chute moyenne à haute (de 30m à >300m), et les centrales hydrauliques au fil de l'eau (Run of River - RoR), caractérisées par une implantation le long de grands fleuves ou grandes rivières, avec un débit très fort et une faible hauteur de chute (moins de 30m). Localisés au sein de la région PACA, nous nous sommes concentrés sur les barrages au fil de l'eau, prépondérants dans la région.(\textbf{\textit{rajouter deux trois chiffres dessus}})Étant placés directement le long des fleuves et rivières, ils dépendent très fortement des conditions climatiques.
...
Après s’être entrainé sur des données pour les années 2015 à 2022, puis avoir validé en testant nos modèles sur les données de 2023, nous avons appliqué cela aux années futures, en essayant de modéliser des situations précises à l’horizon 2050. 
...
Les données dont nous disposons correspondent aux valeurs de températures et de précipitations au niveau régional, ainsi que les CF au niveau national, pour des données journalières allant du premier janvier 2015 au 31 décembre 2023. On cherche donc à prédire le facteur de capacité national à partir des données météorologiques locales.\\
...
\begin{table}[H]
    ...
    \textbf{Modèle} & \textbf{R}^2 & \textbf{nMAE} & \textbf{MSE} & \textbf{nRMSE} \\
    \hline % Ligne horizontale séparant les en-têtes des données
    Ridge Regression & 0.5831 & 0.1429 & \textbf{MSE} & 0.1839 \\
    \hline
    Random Forest & & & & \\
    \hline
    Gradient Boosting &0.6637 &0.1333 & & 0.1652\\
    \hline
    LSTM &0.7126 &0.1172 & &0.1527 \\
    \hline % Ligne horizontale inférieure
    \end{tabular}
    \caption{Tableau récapitulatif des meilleurs résultats par modèle en 2023} % Caption plus claire
